...
export default function Scanner() {
  ...
  async function markSticker(code, status) {
    ...
    showToast(status === 'HAVE' ? 'Marcada: ' + result.pais + ' #' + result.num : 'Repetida!')
    setResult(null)
  }
    ...
    <div style={{ minHeight: '100vh', background: '#050a0f', color: 'white', fontFamily: 'Barlow' }}>
      ...
      <div style={{ maxWidth: '600px', margin: '0 auto', padding: '20px' }}>
        <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '14px', padding: '14px', marginBottom: '16px' }}>
          <div style={{ fontFamily: 'Barlow Condensed', fontSize: '15px', fontWeight: '700', marginBottom: '8px', color: '#F5C518' }}>Como usar</div>
          <div style={{ fontSize: '12px', color: 'rgba(255,255,255,0.6)', lineHeight: '1.8' }}>
            1. Clique em INICIAR SCANNER<br/>
            2. Aponte a camera para o codigo da figurinha<br/>
            3. O sistema detecta automaticamente<br/>
            4. Confirme TENHO ou REPETIDA
          </div>
        </div>
        <div style={{ position: 'relative', borderRadius: '16px', overflow: 'hidden', background: '#0a0a0a', border: '2px solid rgba(255,255,255,0.1)', marginBottom: '16px', aspectRatio: '16/9' }}>
          ...
          {scanning && (
            <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none' }}>
              <div style={{ position: 'absolute', left: '20%', right: '20%', top: '65%', bottom: '18%', border: '2px solid rgba(232,23,93,0.8)', borderRadius: '8px', boxShadow: '0 0 0 9999px rgba(0,0,0,0.45)' }} />
              <div style={{ position: 'absolute', bottom: '20%', left: '50%', transform: 'translateX(-50%)', fontSize: '11px', color: 'white', fontWeight: '700', whiteSpace: 'nowrap', background: 'rgba(232,23,93,0.7)', padding: '3px 12px', borderRadius: '99px' }}>
        ...
        {result && (
          <div style={{ background: 'rgba(34,197,94,0.1)', border: '2px solid #22C55E', borderRadius: '16px', padding: '16px', marginBottom: '16px', textAlign: 'center' }}>
            <div style={{ fontFamily: 'Barlow Condensed', fontSize: '13px', fontWeight: '700', color: '#22C55E', letterSpacing: '1px', marginBottom: '6px' }}>FIGURINHA DETECTADA!</div>
            <div style={{ fontFamily: 'Barlow Condensed', fontSize: '28px', fontWeight: '900', color: 'white', marginBottom: '4px' }}>{result.pais} #{result.num}</div>
            <div style={{ fontSize: '12px', color: 'rgba(255,255,255,0.4)', marginBottom: '14px' }}>{result.code}</div>
        ...
        {lastScanned.length > 0 && (
          <div>
            <div style={{ fontFamily: 'Barlow Condensed', fontSize: '13px', fontWeight: '700', letterSpacing: '1px', color: 'rgba(255,255,255,0.4)', marginBottom: '10px' }}>HISTORICO DA SESSAO</div>
            {lastScanned.map((item, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '8px 12px', background: 'rgba(255,255,255,0.03)', borderRadius: '10px', marginBottom: '6px', border: '1px solid rgba(255,255,255,0.06)' }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: '10px', color: 'rgba(255,255,255,0.4)' }}>{item.status === 'HAVE' ? 'TENHO' : 'REPETIDA'} - {item.time}</div>
                </div>
              </div>
            ))}
          </div>
