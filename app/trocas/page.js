...
export default function Trocas() {
  ...
  const [form, setForm] = useState({ offer:'', want:'', state:'SP', contact:'', name:'', method:'correio' })
  ...
  async function postTrade(e) {
    ...
    setPosting(true)
    setTimeout(() => {
      setTrades([{ id: Date.now(), ...form, time: 'Agora mesmo', rep: 5.0 }, ...trades])
      setShowForm(false)
      setPosting(false)
      setForm({ offer:'', want:'', state:'SP', contact:'', name:'', method:'correio' })
    }, 800)
  }
    ...
    <div style={{minHeight:'100vh',background:'#0a0a0a',color:'white',fontFamily:'Barlow'}}>
      ...
      <div style={{maxWidth:'700px',margin:'0 auto',padding:'16px'}}>
        ...
        <div style={{background:'linear-gradient(135deg,#E8175D,#6B2FFA)',borderRadius:'16px',padding:'18px',marginBottom:'16px',position:'relative',overflow:'hidden'}}>
          ...
          <div style={{fontSize:'12px',color:'rgba(255,255,255,0.7)',marginBottom:'14px'}}>Conectando colecionadores do Brasil todo</div>
          <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'8px'}}>
            {[['1.247','COLECIONADORES'],['3.891','TROCAS FEITAS'],['89','ONLINE AGORA']].map(([v,l])=>(
        ...
        <div onClick={() => setShowForm(true)}
          ...
          <div style={{width:'42px',height:'42px',borderRadius:'11px',background:'#E8175D',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'20px',flexShrink:0}}>+</div>
          <div>
        ...
        {trades.length === 0 ? (
          ...
          <div key={t.id} style={{background:'#141414',border:'1px solid rgba(255,255,255,0.08)',borderRadius:'14px',padding:'14px',marginBottom:'10px'}}>
            <div style={{display:'flex',alignItems:'center',gap:'8px',marginBottom:'10px'}}>
              <div style={{width:'36px',height:'36px',borderRadius:'50%',background:'#E8175D',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'16px'}}>🦁</div>
              <div style={{flex:1}}>
                <div style={{fontFamily:'Barlow Condensed',fontSize:'15px',fontWeight:'700'}}>{t.name}</div>
                <div style={{fontSize:'10px',color:'rgba(255,255,255,0.4)'}}>{t.state} · {t.method === 'correio' ? '📬 Correio' : '🤝 Presencial'}</div>
              </div>
              <div style={{textAlign:'right'}}>
                <div style={{color:'#F5C518',fontSize:'12px'}}>★★★★★</div>
                <div style={{fontFamily:'Barlow Condensed',fontSize:'14px',fontWeight:'900'}}>{t.rep}</div>
              </div>
            </div>
            <div style={{display:'grid',gridTemplateColumns:'1fr auto 1fr',gap:'8px',marginBottom:'10px'}}>
              <div style={{background:'#1e1e1e',borderRadius:'8px',padding:'8px'}}>
                <div style={{fontSize:'8px',fontWeight:'700',letterSpacing:'1px',color:'#22C55E',marginBottom:'4px'}}>OFERECE</div>
                <div style={{fontSize:'11px',color:'rgba(255,255,255,0.8)'}}>{t.offer}</div>
              </div>
              <div style={{display:'flex',alignItems:'center',fontFamily:'Barlow Condensed',fontSize:'20px',fontWeight:'900',color:'rgba(255,255,255,0.3)'}}>⇄</div>
              <div style={{background:'#1e1e1e',borderRadius:'8px',padding:'8px'}}>
                <div style={{fontSize:'8px',fontWeight:'700',letterSpacing:'1px',color:'#1A56DB',marginBottom:'4px'}}>PRECISA</div>
                <div style={{fontSize:'11px',color:'rgba(255,255,255,0.8)'}}>{t.want}</div>
              </div>
            </div>
            <div style={{display:'flex',alignItems:'center',justifyContent:'space-between'}}>
              <div style={{fontSize:'10px',color:'rgba(255,255,255,0.3)'}}>⏱ há {t.time}</div>
              <button style={{padding:'7px 16px',background:'#E8175D',border:'none',borderRadius:'99px',color:'white',fontFamily:'Barlow Condensed',fontSize:'12px',fontWeight:'700',cursor:'pointer',letterSpacing:'.5px'}}>
                CONTATAR
              </button>
            </div>
          </div>
      ...
      {/* FORM MODAL */}
      {showForm && (
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.7)',display:'flex',alignItems:'flex-start',justifyContent:'center',padding:'16px',zIndex:200,overflowY:'auto'}}>
          <div style={{background:'#141414',border:'2px solid #0a0a0a',borderRadius:'20px',width:'100%',maxWidth:'400px',overflow:'hidden'}}>
            <div style={{height:'4px',display:'flex'}}>{COLORS.map((c,i)=><span key={i} style={{flex:1,background:c}}/>)}</div>
            ...
            <form onSubmit={postTrade} style={{padding:'16px'}}>
              <div style={{marginBottom:'10px'}}>
                <label style={{display:'block',fontSize:'9px',fontWeight:'700',letterSpacing:'.8px',color:'rgba(255,255,255,0.5)',marginBottom:'5px'}}>MÉTODO</label>
              ...
              {[['SEU NOME *','name','Como te chamam?'],['O QUE OFERECE *','offer','Ex: BRA 7 12 18, ARG 3...'],['O QUE PRECISA *','want','Ex: FRA 1 2, ESP 8...'],['CONTATO *','contact','WhatsApp ou @instagram']].map(([l,k,p]) => (
              ...
              <div style={{marginBottom:'14px'}}>
                <label style={{display:'block',fontSize:'9px',fontWeight:'700',letterSpacing:'.8px',color:'rgba(255,255,255,0.5)',marginBottom:'5px'}}>ESTADO</label>
